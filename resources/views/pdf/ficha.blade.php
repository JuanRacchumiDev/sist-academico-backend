...
$nombreCompleto = !empty($alumno->nombre_completo) ? $alumno->nombre_completo : "Invitado";
$email = !empty($alumno->email) ? $alumno->email : "anonimo@example.com";
$numeroDocumento = !empty($alumno->numero_documento) ? $alumno->numero_documento : "SIN_DOCUMENTO";
$telefono = !empty($alumno->telefono) ? $alumno->telefono : '----';
$direccion = !empty($sede->nombre) ? $sede->nombre : '----';
?>
<!DOCTYPE html>
<html>
<head>
...
<body>
    ...
    <p class="mb-10">Yo, <strong class="text-underline">{{ $nombreCompleto }}</strong> identificado (a) con D.N.I. N° {{ $numeroDocumento }} autorizo a través de la grabación de voz la participación en los programas de capacitación universitaria desarrollado con la {{ $empresa['razon_social'] }}.</p>
    <div class="mb-10">
        <?php $contador = 0; ?>
        <?php foreach($detalles as $item): ?>
            ...
            <p class="w-100 mb-5">
                ...
                <span class="w-70 border-solid p-5 d-inline-block">{{ $item->nombre_programa }}</span>
            </p>
        <?php endforeach; ?>
    </div>
    <div class="w-100 d-block mb-10">
        <div class="w-49 d-inline-block">
            <div class="w-100 d-block">
                ...
                <span class="w-50 d-inline-block">{{ $email }}</span>
            </div>
        </div>
        <div class="w-49 d-inline-block">
            <span class="w-30 d-inline-block">TELÉFONO</span>
            <span class="w-60 d-inline-block">{{ $telefono }}</span>
        </div>
    </div>
    <div class="w-100 d-block">
        <div class="w-49 d-inline-block">
            <span class="w-40 d-inline-block">DIRECCIÓN</span>
            <span class="w-50 d-inline-block">{{ $direccion }}</span>
        </div>
        <div class="w-49 d-inline-block">
